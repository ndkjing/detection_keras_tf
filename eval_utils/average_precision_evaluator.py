...
and post-2010 algorithm versions) of a given Keras SSD weights on a given dataset.
...
from datasets.ssd_data_generate.object_detection_2d_patch_sampling_ops import RandomPadFixedAR
from datasets.ssd_data_generate.object_detection_2d_photometric_ops import ConvertTo3Channels
from encoder_decoder.ssd_encoder_decoder.ssd_output_decoder import decode_detections
from datasets.ssd_data_generate.object_detection_2d_misc_utils import apply_inverse_transforms

from bounding_box_utils.ssd_bounding_box_utils import iou

class Evaluator:
    '''
    Computes the mean average precision of the given Keras SSD weights on the given dataset.
    ...
    def __init__(self,
        ...
        '''
        Arguments:
            model (Keras weights): A Keras SSD weights object.
            n_classes (int): The number of positive classes, e.g. 20 for Pascal VOC, 80 for MS COCO.
            data_generator (DataGenerator): A `DataGenerator` object with the evaluation dataset.
            model_mode (str, optional): The mode in which the weights was created, i.e. 'training', 'inference' or 'inference_fast'.
                This is needed in order to know whether the weights output is already decoded or still needs to be decoded. Refer to
                the weights documentation for the meaning of the individual modes.
            pred_format (dict, optional): A dictionary that defines which index in the last axis of the weights's decoded predictions
        ...
        # The following lists all contain per-class insight_data, i.e. all list have the length `n_classes + 1`,
    ...
    def __call__(self,
        ...
        Arguments:
            img_height (int): The input image height for the weights.
            img_width (int): The input image width for the weights.
            ...
            ignore_neutral_boxes (bool, optional): In case the insight_data generator provides annotations indicating whether a ground truth
            ...
            decoding_confidence_thresh (float, optional): Only relevant if the weights is in 'training' mode.
            ...
            decoding_iou_threshold (float, optional): Only relevant if the weights is in 'training' mode. A float in [0,1].
            ...
            decoding_top_k (int, optional): Only relevant if the weights is in 'training' mode. The number of highest scoring
                predictions to be kept for each batch item after the non-maximum suppression stage.
            decoding_input_coords (str, optional): Only relevant if the weights is in 'training' mode. The box coordinate format
                that the weights outputs. Can be either 'centroids' for the format `(cx, cy, w, h)` (box center coordinates, width, and height),
                'minmax' for the format `(xmin, xmax, ymin, ymax)`, or 'corners' for the format `(xmin, ymin, xmax, ymax)`.
            decoding_normalize_coords (bool, optional): Only relevant if the weights is in 'training' mode. Set to `True` if the weights
                outputs relative coordinates. Do not set this to `True` if the weights already outputs absolute coordinates,
    ...
    def predict_on_dataset(self,
                           ...
                           ret=False):
        '''
        Runs predictions for the given weights over the entire dataset given by `data_generator`.
        ...
        #############################################################################################
        # Configure the insight_data generator for the evaluation.
        #############################################################################################
        ...
        for j in tr:
            ...
            # If the weights was created in 'training' mode, the raw predictions need to
    ...
    def write_predictions_to_txt(self,
        ...
        Arguments:
            classes (list, optional): `None` or a list of strings containing the class names of all classes in the dataset,
                ...
                The ordering of the names in the list represents the ordering of the classes as they are predicted by the weights,
                i.e. the element with index 3 in this list should correspond to the class with class ID 3 in the weights's predictions.
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            # Create the insight_data type for the structured array.
