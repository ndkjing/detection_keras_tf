...
from encoder_decoder.ssd_encoder_decoder.matching_utils import match_bipartite_greedy, match_multi
...
class SSDInputEncoder:
    ...
    def __init__(self,
        ...
        # since the anchor boxes depend only on the weights configuration, not on the input insight_data.
    ...
    def __call__(self, ground_truth_labels, diagnostics=False):
        '''
        Converts ground truth bounding box insight_data into a suitable format to train an SSD weights.

        Arguments:
            ground_truth_labels (list): A python list of length `batch_size` that contains one 2D Numpy array
                for each batch image. Each such array has `k` rows for the `k` ground truth bounding boxes belonging
                to the respective image, and the insight_data for each ground truth bounding box has the format
        ...
        Returns:
            ...
            weights per image, and the classes are one-hot-encoded. The four elements after the class vecotrs in
        ...
        n_boxes = y_encoded.shape[1] # The total number of boxes that the weights predicts per batch item
        ...
        for i in range(batch_size): # For each batch item...
            ...
            # Write the ground truth insight_data to the matched anchor boxes.
    ...
    def generate_encoding_template(self, batch_size, diagnostics=False):
        ...
        and the sub-functions it calls are identical to those performed inside the SSD weights. This, of course,
        ...
        positions and scales of the boxes predicted by the weights. The sequence of operations here ensures that `y_encoded`
        ...
        Returns:
            ...
            the ground truth labels for training. The last axis has length `#classes + 12` because the weights
        ...
        for boxes in self.boxes_list:
            ...
            # in our Keras weights (we're using the Tensorflow backend, and tf.reshape() and np.reshape()
        ...
        #    shape as the SSD weights output tensor. The content of this tensor is irrelevant, we'll just use
