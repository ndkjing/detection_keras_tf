'''
Includes:
* Functions to decode and filter raw SSD weights output. These are only needed if the
  SSD weights does not have a `DecodeDetections` layer.
...
from bounding_box_utils.ssd_bounding_box_utils import iou, convert_coordinates
...
def decode_detections(y_pred,
                      ...
                      border_pixels='half'):
    '''
    Convert weights prediction output back to a format that contains only the positive box predictions
    ...
    For a slightly different and more efficient alternative to decode raw weights output that performs
    non-maximum suppresion globally instead of per class, see `decode_detections_fast()` below.

    Arguments:
        y_pred (array): The prediction output of the SSD weights, expected to be a Numpy array
            of shape `(batch_size, #boxes, #classes + 4 + 4 + 4)`, where `#boxes` is the total number of
            boxes predicted by the weights per image and the last axis contains
        ...
        input_coords (str, optional): The box coordinate format that the weights outputs. Can be either 'centroids'
            for the format `(cx, cy, w, h)` (box center coordinates, width, and height), 'minmax' for the format
            `(xmin, xmax, ymin, ymax)`, or 'corners' for the format `(xmin, ymin, xmax, ymax)`.
        normalize_coords (bool, optional): Set to `True` if the weights outputs relative coordinates (i.e. coordinates in [0,1])
            and you wish to transform these relative coordinates back to absolute coordinates. If the weights outputs
            relative coordinates, but you do not want to convert them back to absolute coordinates, set this to `False`.
            Do not set this to `True` if the weights already outputs absolute coordinates, as that would result in incorrect
    ...
    # 2: If the weights predicts normalized box coordinates and they are supposed to be converted back to absolute coordinates, do that
...
def decode_detections_fast(y_pred,
    ...
    # 3: If the weights predicts normalized box coordinates and they are supposed to be converted back to absolute coordinates, do that
    if normalize_coords:
...
# unless you need to debug your weights, of course.
...
def decode_detections_debug(y_pred,
    ...
    That is, in addition to the usual insight_data, each predicted box has the internal index of that box within
    the weights (`box_id`) prepended to it. This allows you to know exactly which part of the weights made a given
    ...
    for batch_item in y_pred_decoded_raw: # `batch_item` has shape `[n_boxes, n_classes + 4 coords]`
        ...
        for class_id in range(1, n_classes): # For each class except the background class (which has class ID 0)...
            ...
            if threshold_met.shape[0] > 0: # If any boxes made the threshold...
                ...
                maxima_output[:,2:] = maxima[:,1:] # ...and write the rest of the maxima insight_data to the other columns...
                pred.append(maxima_output) # ...and append the maxima for this class to the list of maxima for this batch item.
        # Once we're through with all classes, keep only the `top_k` maxima with the highest scores
        pred = np.concatenate(pred, axis=0)
...
def get_pred_layers(y_pred_decoded, num_boxes_per_pred_layer):
    ...
    Arguments:
        y_pred_decoded (array): The decoded weights output tensor. Must have been
